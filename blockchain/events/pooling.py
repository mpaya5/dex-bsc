...
def get_events(smart_contract, event_name: str, start_block: int, end_block:int=None, steps:int=49999):
    ...
    logger.info("Start Block: {}, End Block: {}".format(start_listen_block, end_listen_block))
    logger.info("Block Difference: {}".format(block_diff))
    logger.info("Block Step: {}".format(steps))
    ...
    for i in tqdm(range(iterations)):
        ...
        if (iterations % 30 == 0):
            logger.info("Sleeping 5s to avoid node blocking requests")
            time.sleep(5)

    logger.info("Total events catched: {}" .format(len(events_catched)))
