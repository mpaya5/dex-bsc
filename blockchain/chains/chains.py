...
from dotenv impact load_dotenv
load_dotenv()

KEY_CHAIN = os.getenv('KEY_CHAIN')
...
class Chain:
    ...
    def get_chain_info(self):
        logger.info("Connected: {}".format(self.w3.is_connected()))
        logger.info("Chain name: {}".format(self.name))
        logger.info("Chain id: {}".format(self.chain_id))
        logger.info("Max gwei: {}".format(self.max_gwei))
        logger.info("Extra gwei: {}\n".format(self.extra_gwei))
...
class EthereumChain(Chain):
    def __init__(self):
        super().__init__(web_url=f'https://mainnet.infura.io/v3/{KEY_CHAIN}',
...
class RopstenChain(Chain):
    def __init__(self):
        super().__init__(web_url=f'https://ropsten.infura.io/v3/{KEY_CHAIN}',
                         name='ropsten', coin='ETH', cg_ticker='')


class RinkebyChain(Chain):
    def __init__(self):
        super().__init__(web_url=f'https://rinkeby.infura.io/v3/{KEY_CHAIN}',
